...
export function EmailItem({ msg, index, ref, isFocused, isMarkReadLoading, isFlagLoading, openMailDetail, handleToggleRead, handleToggleFlag }: EmailItemProps) {
  ...
  return (
    <div
      ...
      className={cn("group relative flex cursor-pointer gap-3 px-4 py-3 hover:bg-slate-50", isFocused && "border-l-4 border-l-blue-500 bg-blue-50/70 pl-3")}
    >
      {/* Avatar */}
      <div
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-0.5">
        ...
        <div className="flex items-end justify-between gap-2">
          ...
          >
            {isFlagLoading ? <Loader2 className="size-4 animate-spin" /> : <FlagIcon isFlagged={isFlagged} className="size-4" />}
          </button>
        </div>
      </div>
    </div>
